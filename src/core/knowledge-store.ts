...
//   3. Trusted, APP-OWNED identity: `document_id` is derived HERE from the
//      normalized SOURCE URI (7c: source-addressed identity), and `chunk_id`
//      from server-computed content hashes — never model-generated — so later
...
CREATE TABLE IF NOT EXISTS ${META_TABLE} (
  ...
  value TEXT NOT NULL
      ...
      // stable identity = sha256 of the normalized source_uri (see
      // computeDocumentId / normalizeSourceUri) — 7c source-addressed identity.
      // content_hash is the SEPARATE hash of the current content VERSION (see
      // computeContentHash), so re-ingesting the same source with changed
      // content is an in-place UPDATE of the same document_id (7c). source_uri
      // is the identity input; it is kept nullable at the SQL level for
      // forward-only, additive-migration hygiene (no destructive NOT NULL
      // rewrite) and enforced non-empty in the app layer (see insertDocument /
      // normalizeSourceUri). owner_id is nullable now but present so later
...
// ---------------------------------------------------------------------------
// APP-OWNED IDENTITY (7c: SOURCE-ADDRESSED). The SOURCE is the entity; the
// content is its current VERSION. `document_id` is derived HERE from the
// normalized source_uri, and `content_hash` from the content — both server-owned
// and never model-generated — so trusted citations/authorization rest on stable
// IDs (exit criterion 3 groundwork) and update/delete have first-class semantics
// (exit criterion 2).
// ---------------------------------------------------------------------------

// Typed, redaction-safe error raised when a required source_uri is missing,
// empty, or whitespace-only. The source_uri IS the document's identity (7c), so
// it can never be absent. Thrown by normalizeSourceUri (and therefore by
// computeDocumentId / insertDocument) BEFORE any write, so nothing is persisted.
export class InvalidSourceUriError extends Error {
  constructor() {
    super(
      redactText(
        "A non-empty source_uri is required: the source URI is the document's " +
          "app-owned identity and cannot be empty or whitespace-only."
      )
    );
...
// Normalize a source_uri BEFORE it is hashed into identity. The normalization is
// intentionally MINIMAL and precisely specified:
//   1. Unicode NFC (canonical composition) — so two byte-different but
//      canonically-equivalent URIs map to ONE identity.
//   2. Trim leading/trailing whitespace — surrounding whitespace is never
//      semantically part of a URI/path.
// It DOES NOT case-fold: URIs and filesystem paths are case-sensitive in the
// general case (e.g. POSIX paths, URL path/query components), so lowercasing
// could wrongly merge two distinct sources. An empty or whitespace-only input
// (after trim) is REJECTED with InvalidSourceUriError — a source must exist.
// This is the single documented normalization used to derive every document_id.
export function normalizeSourceUri(sourceUri: string): string {
  if (typeof sourceUri !== "string") {
    throw new InvalidSourceUriError();
  }
  const normalized = sourceUri.normalize("NFC").trim();
  if (normalized === "") {
    throw new InvalidSourceUriError();
  }
  return normalized;
...
// (line-ending style, unstable Unicode composition) do NOT change the content
// VERSION hash. This is the single, documented normalization used for BOTH the
// document's content_hash and chunk content hashes. Kept intentionally
// conservative: normalize Unicode to NFC and collapse CRLF/CR to LF. It does NOT
// trim or reflow, so meaningful whitespace is preserved. Exported for chunking
// and ingestion to reuse the EXACT same normalization.
...
// content VERSION hash stored as `documents.content_hash` and as each chunk's
// content_hash. It is NO LONGER the document identity (that is source-addressed
// now — see computeDocumentId); a document_id and its content_hash are genuinely
// DIFFERENT values.
export function computeContentHash(content: string): string {
  return createHash("sha256").update(normalizeContent(content), "utf8").digest("hex");
}

// The app-owned stable document identity = sha256 of the NORMALIZED source_uri.
// Identical normalized source URIs ALWAYS yield the same document_id
// (deterministic), which is what makes 7c re-ingestion an in-place UPDATE of the
// same document rather than a new row.
//
// DESIGN DECISION (7c — SOURCE-ADDRESSED identity): a document's identity is the
// SOURCE, not its content. document_id == sha256(normalizeSourceUri(source_uri)),
// INDEPENDENT of content. Consequences, all by design:
//   - Two DIFFERENT source URIs with byte-for-byte identical content are now TWO
//     distinct documents (NO content collision / no dedup-by-content).
//   - The SAME source re-ingested with DIFFERENT content is an UPDATE of the
//     SAME document_id (content_hash changes; identity does not).
// This SUPERSEDES the 7a/7b content-addressed model. `content_hash` remains as a
// separate column and is now the current VERSION marker, not identity.
export function computeDocumentId(sourceUri: string): string {
  return createHash("sha256")
    .update(normalizeSourceUri(sourceUri), "utf8")
    .digest("hex");
}
...
// Input to insertDocument. `sourceUri` is REQUIRED — it is the document's
// app-owned identity (7c source-addressed), validated + normalized via
// normalizeSourceUri (throws InvalidSourceUriError when empty/whitespace-only).
// The store derives document_id from the normalized source_uri and content_hash
// from `content` (never trusts a caller/model id). `ownerId` is optional
// (authorization-ready, nullable now).
...
export interface KnowledgeStore {
  ...
  // Insert a document. Derives document_id from the REQUIRED, normalized
  // `sourceUri` (7c identity) and content_hash from `content`. Throws
  // InvalidSourceUriError on an empty source and on a duplicate document_id (a
  // storage primitive; idempotent upsert-by-source is 7c ingestion).
  ...
  // Convenience: fetch the document owning `sourceUri` (== getDocument of the
  // source's derived document_id), or null. Throws InvalidSourceUriError on an
  // empty source.
  getDocumentBySource(sourceUri: string): KnowledgeDocument | null;
  ...
  // Convenience: hard-delete the document owning `sourceUri` (7c source-scoped
  // delete), same atomic cascade as deleteDocument. Returns true if removed,
  // false if no such source. Throws InvalidSourceUriError on an empty source.
  deleteDocumentBySource(sourceUri: string): boolean;
  // Delete ALL of a document's chunks AND their vec0 vectors atomically, WITHOUT
  // touching the document row (7c chunk-replacement primitive for in-place
  // update: delete-all-then-insert). Returns the number of chunk rows removed.
  deleteChunks(documentId: string): number;
  // Update a document's content_hash to the hash of `content` and BUMP
  // updated_at, PRESERVING created_at (7c in-place version update). Returns the
  // refreshed document, or null if no such document. Does NOT touch chunks —
  // callers pair this with deleteChunks + insertChunk inside one transaction.
  touchDocumentContent(documentId: string, content: string): KnowledgeDocument | null;
...
export function createKnowledgeStore(
  ...
  const deleteChunksStmt = db.prepare(
    `DELETE FROM ${CHUNKS_TABLE} WHERE document_id = ?`
  );
  const touchDocumentStmt = db.prepare(
    `UPDATE ${DOCUMENTS_TABLE}
        SET content_hash = @content_hash, updated_at = @updated_at
  ...
  // Atomic hard delete by id: remove the document row (chunks vanish via ON
  // DELETE CASCADE) AND explicitly delete each chunk's vector from the vec0
  // table (a virtual table is not reached by a relational FK cascade). All in
  // one transaction so no orphaned vectors can remain. Shared by both
  // deleteDocument and deleteDocumentBySource. Returns false (clean no-op) when
  // no such document exists.
  function deleteDocumentById(documentId: string): boolean {
  ...
  return {
    ...
    insertDocument(input) {
      const now = Date.now();
      // The REQUIRED source_uri is the identity (7c). Normalize + validate it
      // (throws InvalidSourceUriError on empty) and derive document_id from it;
      // content_hash is the SEPARATE content-version hash. The normalized URI is
      // what we persist, so getDocumentBySource round-trips deterministically.
      const normalizedSourceUri = normalizeSourceUri(input.sourceUri);
      const documentId = computeDocumentId(normalizedSourceUri);
    ...
    getDocumentBySource(sourceUri) {
      const row = getDocumentStmt.get(computeDocumentId(sourceUri));
      return row ? rowToDocument(row) : null;
    },
    ...
    deleteDocumentBySource(sourceUri) {
      return deleteDocumentById(computeDocumentId(sourceUri));
    },

    deleteChunks(documentId) {
      // Atomic chunk replacement primitive (7c): remove ALL of the document's
      // chunk rows AND their vec0 vectors in ONE transaction, WITHOUT touching
      // the document row. A virtual table is not reached by a relational FK
      // cascade, so vectors are cleaned explicitly here — leaving zero orphans.
      return db.transaction(() => {
        const vecRows = selectVecRowidsForDocumentStmt.all(documentId);
        const info = deleteChunksStmt.run(documentId);
        for (const { vec_rowid } of vecRows) {
          deleteVectorStmt.run(BigInt(vec_rowid));
        }
        return info.changes;
    ...
    touchDocumentContent(documentId, content) {
      // In-place version bump (7c): set the new content_hash + updated_at while
      // PRESERVING created_at (the UPDATE never touches it). Chunk replacement is
      // the caller's responsibility (pair with deleteChunks + insertChunk in one
      // transaction).
      const info = touchDocumentStmt.run({
