...
- **4d — Local tracing with redaction.** Status: Active (in progress / next). Add local, self-contained tracing with secret/PII redaction and documented observability boundaries. No external SaaS tracing backend.
...
- Vitest is in place with deterministic offline Phase 1–3 regression coverage (4a); the full suite is 57 offline tests. `npm run test:connection` remains the only automated runtime connectivity smoke check and makes a live request.
