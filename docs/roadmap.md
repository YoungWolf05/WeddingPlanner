...
- **5e — Schema versioning & migration.** Status: Active (in progress / next). Add an app-owned schema version marker plus a forward-only migration runner for the application's own tables (the `threads` table and any future app tables), replacing the bare `CREATE TABLE IF NOT EXISTS`. Shared-DB-safe: the runner must not collide with or attempt to migrate the LangGraph-owned checkpoint tables, which remain the library's responsibility and stay guarded by the version-pin test. Forward-only safety: fail loudly if the DB was written by a newer app schema version than the running code understands. Migrations are atomic and data-preserving; deterministic offline tests cover fresh-init, forward migration, and the newer-than-supported refusal. Rationale: to genuinely satisfy Phase 5 exit criterion (3)'s "schema migration behavior" clause instead of deferring it.
...
- [ ] Retention/deletion and schema migration behavior are implemented and tested. (Retention/deletion is implemented and tested in 5d via the atomic, owner-scopeable `ThreadStore.pruneThreads` hook; the "schema migration" clause is being addressed by increment 5e — app-owned schema versioning + forward-only migration runner — and is not yet ticked pending 5e delivery.)
...
- Exit criterion (3)'s retention/deletion clause is satisfied by 5d; its schema-migration clause is deliberately NOT yet met — the `threads` table still uses `CREATE TABLE IF NOT EXISTS` and the LangGraph checkpoint tables rely on the library's lazy setup pinned to `@langchain/langgraph-checkpoint-sqlite` 1.0.3, with no app-owned versioning/migration path. Increment 5e adds an app-owned schema version marker + forward-only migration runner (shared-DB-safe, forward-only-fail-loud, atomic/data-preserving) to close that clause before Phase 5 closeout.
