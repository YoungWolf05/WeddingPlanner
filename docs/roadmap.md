...
Phase 5 is complete. Phase 6 is user-approved and in progress (increment 6a complete; 6b active). Phase 7 is the next proposed phase and is not active until the user approves its activation.
...
- **6b — Safe tools.** Status: Active (in progress / next). Implement `days_until(date)` and `split_budget(total, categories?)` as pure, deterministic, read-only functions (no filesystem/network/writes) plus `tool()` wrappers with Zod input schemas; unit-test the pure functions directly (boundary/validation cases). No side-effecting/write tools in Phase 6. The 6a over-allocation strict rule (`validateBudgetAllocation` / `budgetPlanStrictSchema`) is the intended consumer of the 6b budget tool's domain layer.
- **6c — Tool-loop agent.** Status: Planned. Wire a LangChain `createAgent` tool-calling path so the bot actually calls the 6b tools, keeping the existing conversational graph for plain chat (no hand-wired `bindTools` in the current graph). End-to-end exit-criterion test (e.g., "how many days until 2026-12-12 and split a $30k budget") runs offline against a mocked model emitting tool calls.
