...
- **5d — Hardening & docs.** Status: Active (in progress / next). Concurrency behavior, restart/durability validation at the HTTP layer, retention/deletion policy, README/AGENTS/roadmap alignment, and Phase 5 closeout. **5d hardening backlog (deferred code-review items carried from 5c — defense-in-depth/hardening, not current defects):**
  - **R1 — HTTP server timeouts.** Add request/idle/headers timeouts to mitigate slow-client/slowloris and long-lived idle SSE connections; the body is currently size-capped (413) but not time-bounded.
  - **R3 — prototype-pollution hardening.** Add defense-in-depth handling for JSON body keys (reject/ignore `__proto__`/`constructor`/`prototype`), or document the "read named fields only, never merge" invariant. Not currently exploitable.
  - **R4 — 413 drain-to-EOF trade-off.** Revisit the oversized-body drain-to-EOF behavior together with R1's timeouts.
