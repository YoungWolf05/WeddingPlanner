...
Phase 5 is complete. Phase 6 is user-approved and in progress (increments 6a and 6b complete; 6c active). Phase 7 is the next proposed phase and is not active until the user approves its activation.
...
- **6c — Tool-loop agent.** Status: Active (in progress / next). Wire a LangChain `createAgent` tool-calling path so the bot actually calls the 6b `weddingTools`, keeping the existing conversational graph for plain chat (no hand-wired `bindTools` in the current graph). End-to-end exit-criterion test (e.g., "how many days until 2026-12-12 and split a $30k budget" exercising both `days_until` and `split_budget`) runs offline against a mocked model emitting tool calls.
- **6d — Hardening + docs + closeout.** Status: Planned. Add per-tool timeouts and typed/bounded error handling, capability/model-selection notes, `AGENTS.md`/`README.md`/roadmap alignment, and Phase 6 closeout with recorded verification evidence.
